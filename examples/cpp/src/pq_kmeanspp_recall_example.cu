 ...
 *
 * IVF-PQ recall using PQ codebooks trained by the preprocessing PQ API.
 *
 * The codebooks are installed into a single-list IVF-PQ index with a zero
 * centroid and identity rotation. This evaluates asymmetric distance search
 * directly, without reconstructing the base vectors.
 *
 * The outer initializer is selected with CUVS_KMEANS_INIT:
 *   scalable  (default) scalable k-means++
 *   random              ordinary random initialization
 *
 * Step-8 of scalable k-means++ is selected with CUVS_KMEANS_PP_STEP8:
 *   kmeans++  (default)  kmeansPlusPlus on the oversampled coreset
 *   random               initRandom on the oversampled coreset
 */
...
struct args {
  ...
  int64_t init_size       = 0;
  uint32_t train_per_code = 1024;  // 256 centers * 1024 = 262144 train rows
...
void usage(const char* prog)
{
  std::cerr << "Usage: " << prog << " [options]\n"
            ...
            << "  --eval-base PATH         held-out base.fbin [--base]\n"
            << "  --queries PATH           query.fbin [" << args{}.queries << "]\n"
            << "  --groundtruth PATH       neighbors.ibin [" << args{}.groundtruth << "]\n"
            << "  --pq-bits N              [8]\n"
            << "  --pq-dim N               [64]\n"
            << "  --max-iter N             classic k-means maximum iterations [300]\n"
            << "  --oversampling F         KMeans|| oversampling_factor [2.0]\n"
            << "  --init-size N            rows used to initialize each codebook [0 = all]\n"
            << "  --train-per-code N       max_train_points_per_pq_code [1024]\n"
            << "  --k N                    recall@k [10]\n"
            << "  --seed N                 [42]\n"
            << "  --train-only             build the quantizer; skip IVF-PQ recall\n"
            << "Env: CUVS_KMEANS_INIT=scalable|random\n"
            << "     CUVS_KMEANS_PP_STEP8=random|kmeans++\n"
            << "     CUVS_KMEANS_PP_LOW_DIM=1|0 [1]\n";
}
...
args parse_args(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "-h" || a == "--help") {
      ...
      out.init_size = std::stoll(need(a.c_str()));
...
// PQ codebook is [pq_dim * book_size, pq_len]; IVF-PQ PER_SUBSPACE is
// [pq_dim, pq_len, book_size].
...
int main(int argc, char** argv)
{
  ...
  const char* step8            = std::getenv("CUVS_KMEANS_PP_STEP8");
  const std::string step8_mode = step8 == nullptr ? "kmeans++" : step8;
  const char* init             = std::getenv("CUVS_KMEANS_INIT");
  const bool random_init       = init != nullptr && std::strcmp(init, "random") == 0;
  const std::string init_mode  = random_init ? "random" : "scalable";
  const char* low_dim          = std::getenv("CUVS_KMEANS_PP_LOW_DIM");
  const bool low_dim_enabled   = low_dim == nullptr || std::strcmp(low_dim, "0") != 0;
  const auto total_t0          = std::chrono::steady_clock::now();

  std::cout << "PQ-trained IVF-PQ single-list recall\n"
            << "  base=" << opt.base << "\n"
            << "  queries=" << opt.queries << "\n"
            << "  groundtruth=" << opt.groundtruth << "\n"
            << "  pq_bits=" << opt.pq_bits << " pq_dim=" << opt.pq_dim
            ...
            << "  CUVS_KMEANS_INIT=" << init_mode << "\n"
            << "  CUVS_KMEANS_PP_STEP8=" << step8_mode << "\n"
            << "  CUVS_KMEANS_PP_LOW_DIM=" << (low_dim_enabled ? "1" : "0") << "\n";

  auto [n_base, dim, base_host] = load_bin<float>(opt.base);
  int64_t n_queries             = 0;
  int64_t gt_k                  = 0;
  std::vector<float> query_host;
  std::vector<int32_t> gt_host32;
  if (!opt.train_only) {
    int64_t qdim;
    int64_t n_gt;
    std::tie(n_queries, qdim, query_host) = load_bin<float>(opt.queries);
    std::tie(n_gt, gt_k, gt_host32)        = load_bin<int32_t>(opt.groundtruth);
  ...
  km.init                = random_init ? cuvs::cluster::kmeans::params::InitMethod::Random
                                       : cuvs::cluster::kmeans::params::InitMethod::KMeansPlusPlus;
  km.max_iter            = opt.max_iter;
  km.tol                 = 1e-4;
  km.n_init              = 1;
  km.oversampling_factor = opt.oversampling;
  km.init_size           = opt.init_size;
  ...
  std::optional<cuvs::preprocessing::quantize::pq::quantizer<float>> quantizer;
  std::chrono::steady_clock::time_point t0;
  {
    ...
    quantizer.emplace(cuvs::preprocessing::quantize::pq::build(
  ...
  base_host.clear();
  base_host.shrink_to_fit();

  if (opt.train_only) {
    std::cout << "RESULT step8=" << step8_mode << " train_ms=" << train_ms << "\n";
    return 0;
  }

  const auto& eval_path = opt.eval_base.empty() ? opt.base : opt.eval_base;
  auto [n_eval, eval_dim, eval_host] = load_bin<float>(eval_path);
  if (eval_dim != dim) { throw std::runtime_error("eval base dim != train dim"); }
  if (dim % static_cast<int64_t>(opt.pq_dim) != 0) {
    throw std::runtime_error("dim must be divisible by pq_dim for identity rotation");
  }
  auto eval_dataset = raft::make_device_matrix<float, int64_t>(res, n_eval, dim);
  raft::copy(eval_dataset.data_handle(), eval_host.data(), eval_host.size(), stream);
  ...
  cuvs::neighbors::ivf_pq::extend(
    res, raft::make_const_mdspan(eval_dataset.view()), no_idx, &index);
  raft::resource::sync_stream(res);
  ...
  const auto total_ms =
    std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - total_t0).count();

  std::cout << "RESULT"
            << " init=" << init_mode << " step8=" << step8_mode
            << " low_dim=" << (low_dim_enabled ? "new" : "original")
            << " n_lists=1 n_probes=1 pq_dim=" << opt.pq_dim
            << " train_ms=" << train_ms << " total_ms=" << total_ms << " recall@"
